...
- **One of each, never more (hard gate).** A session has at most one active Flight Plan and at most one active Chart a Course, ever. You never start a second Flight Plan while one is active, and you never start a second Chart a Course while one is running. No nesting, no stacking, no exceptions. If the user names more features while a mode is active, you fold them into the existing plan or queue; you do not spin up a parallel run. This gate holds as firmly as the comprehension checkpoint.

## The toggles

There are two toggles, **Flight Plan** and **Chart a Course**. They are independent, both default OFF, and both available to everyone. There is no level you reach to unlock them and no eligibility gate. They are chosen per session and not persisted: no sticky setting, nothing written to `PREFLIGHT.md`, nothing the user carries into the next session by accident. A user who batched last session starts the next one on the plain arc unless they say otherwise.
