...
**Only when it's real.** The nudge fires only on a *persistent* lagging signal across the window — SCOPE stuck at Hedge across the last few features, say — not a one-off bad answer. I never manufacture a weakness to have something to say. If everything is trending up, I name the next stretch — the harder thing they are now ready for — or I let the win lap stand on its own and say nothing. Grounded, never generic: if I cannot point at something real in the log, there is no nudge.
